...
int
fi_shm_getname(struct shm_ep *ep, void *addr, size_t *addrlen)
{
			      ...
			      fi_shm_connection_t ** remote_connection)
{
	...
	connection_tmp->activity_counter = 0;
	...
	{
		fprintf(stderr, "%s: shmat to %d failed with errno=%d (%s)\n",
			__func__, connection_tmp->sendBlockId, errno, strerror(errno));
		goto err_out_detach;
	}
	...
	if (fi_shm_env.numa_aware)
	{
		...
		if (result != 0)
		{
			FI_SHM_WARNING(("%s: NUMA: Unable to bind send queue memory region %p (%s)\n",
			...
			for (conn_ctrl_pool_iter = 0; conn_ctrl_pool_iter < conn_ctrl_pool_size; conn_ctrl_pool_iter++)
			{
				if (ep->my_conn_ptr->pool[conn_ctrl_pool_iter].isReady)
				{
					...
					if (!connection_tmp)
					{
						...
						FI_SHM_DEBUG(("%s: Connecting backward to %d\n", __func__,
							      connection_ctrl_tmp->connBlockId));
					...
					{
						fprintf(stderr, "%s: shmat to %d failed with errno=%d (%s)\n",
							__func__, connection_tmp->recvBlockId, errno, strerror(errno));
						return FI_SHM_NOK;
					}
...
int
fi_shm_connect(struct shm_ep *ep,
	       void *addr,
	       fi_addr_t *remote_connection)
{
	fi_shm_blockid_t *remote_blockid =
	...
	FI_SHM_DEBUG(("%s: Connecting forward to %d\n", __func__,
		      *remote_blockid));
