...
int
fi_shm_cq_init(struct fi_shm_cq *cq, int cq_format, int cq_size)
{
	cq->format = cq_format;
...
int
fi_shm_cq_fini(struct fi_shm_cq *cq)
{
...
int
fi_shm_ep_bind_cq(struct shm_ep *ep, struct fi_shm_cq *cq, uint64_t flags)
{
	if (flags & FI_SEND) {
		ep->send_cq = cq;
		cq->ep = ep;
...
void
fi_shm_cq_enqueue_event(struct fi_shm_cq *cq, struct fi_shm_cq_event *event)
{
...
static struct fi_shm_cq_event *
fi_shm_cq_dequeue_event(struct fi_shm_cq *cq)
{
	struct fi_shm_cq_event_queue *ceq = &cq->event_queue;
...
fi_shm_cq_create_event(struct fi_shm_cq *cq,
...
fn_recv_overflow:
	...
	fprintf(stderr, "%s: Emergency exit in 10 seconds\n", __func__);
		   ...
		   int count, fi_addr_t *src_addr)
{
	...
	FI_SHM_DEBUG(("%s: poll all count = %d, (active poll count = %d)\n",
		      __func__, count, cq->ep->active_poll_count));
	...
	int min_ativity_connection_counter           = INT_MAX;
	fi_shm_connection_t * min_ativity_connection = NULL;

	FI_SHM_DEBUG(("%s: last_polled_active_connection = %p, last_polled_connection = %p\n",
		      __func__, cq->ep->last_polled_active_connection, cq->ep->last_polled_connection));

	if (cq->ep->active_poll_count)
	{
		if (!LIST_EMPTY(&(cq->ep->active_connection_list)))
		{
			FI_SHM_DEBUG(("%s: poll all (active poll count = %d)\n",
				      __func__, cq->ep->active_poll_count));

			LIST_FOREACH(connection_iterator, &(cq->ep->active_connection_list), active_connections)
			{
				FI_SHM_DEBUG(("%s: poll direction: %d (active)\n",
					      __func__, connection_iterator->connBlockId));

				if (fi_shm_poll_connection(cq, cqe, connection_iterator, src_addr) == FI_SHM_OK)
				{
					FI_SHM_DEBUG(("%s: poll direction: %d (active) successs. activity_counter = %d\n",
						      __func__, connection_iterator->connBlockId, connection_iterator->activity_counter));

					return 1;
				}

				/* Active connections aging logic */

				if (min_ativity_connection_counter > connection_iterator->activity_counter)
				{
					min_ativity_connection_counter = connection_iterator->activity_counter;
					min_ativity_connection = connection_iterator;
				}
			}

			cq->ep->min_activity_connection = min_ativity_connection;
		}
		else
		{
			for (connection_iterator = cq->ep->last_polled_connection;
			     connection_iterator;
			     connection_iterator = SLIST_NEXT(connection_iterator, total_connections))
			{
				FI_SHM_DEBUG(("%s: poll direction: %d (all mode)\n", __func__, connection_iterator->connBlockId));

				if (fi_shm_poll_connection(cq, cqe, connection_iterator, src_addr) == FI_SHM_OK)
				{
					FI_SHM_DEBUG(("%s: poll direction: %d (all mode) success. activity_counter = %d\n",
						      __func__, connection_iterator->connBlockId, connection_iterator->activity_counter));

					cq->ep->last_polled_connection = SLIST_NEXT(connection_iterator, total_connections);
			...
			{
				cq->ep->last_polled_connection = SLIST_FIRST(&(cq->ep->total_connection_list));
			}
		}
	}
	else
	{
		for (connection_iterator = cq->ep->last_flushed_connection;
		     connection_iterator;
		     connection_iterator = SLIST_NEXT(connection_iterator, total_connections))
		{
			FI_SHM_DEBUG(("%s: flush direction: %d (all mode)\n", __func__, connection_iterator->connBlockId));

			if (!TAILQ_EMPTY(&(connection_iterator->postponed_send_request_queue)))
			{
				if (fi_shm_flush_connection_send_queue(cq, connection_iterator, cqe) == FI_SHM_OK)
				{
					if (src_addr)
					{
						*src_addr = (fi_addr_t)connection_iterator;
					}

					cq->ep->last_flushed_connection = SLIST_NEXT(connection_iterator, total_connections);
		...
		if (!connection_iterator)
		{
			cq->ep->last_flushed_connection = SLIST_FIRST(&(cq->ep->total_connection_list));
		}

		FI_SHM_DEBUG(("%s: poll all (all mode)\n", __func__));
		...
		if (cq->ep->min_activity_connection)
		{
			cq->ep->min_activity_connection->activity_counter--;

			if (!cq->ep->min_activity_connection->activity_counter)
			{
				LIST_REMOVE(cq->ep->min_activity_connection, active_connections);
			}
		}
	}

	cq->ep->active_poll_count = (cq->ep->active_poll_count + 1) % fi_shm_env.active_poll_count;

	return 0;
}

ssize_t
fi_shm_cq_readfrom(struct fi_shm_cq *cq, void *buf, size_t count, fi_addr_t *src_addr)
{
	struct fi_shm_cq_event *event;
	int ret;
	ssize_t read_count;
...
ssize_t
fi_shm_cq_readerr(struct fi_shm_cq *cq, struct fi_cq_err_entry *buf, uint64_t flags)
{
	if (cq->pending_error) {
