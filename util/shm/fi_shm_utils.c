...
static int fi_shm_get_int_env(char *name, int default_value)
{
	char *s;
...
int fi_shm_init_ep(struct shm_ep *ep) {
        ...
        fi_shm_env.warning     = fi_shm_get_int_env("OFI_SHM_WARNING", 1);
        ...
        if (fi_shm_env.numa_aware)
        {
                ...
                if (err != 0)
                {
                        fprintf(stderr,"%s: NUMA: Unable to bind connection block memory region %p (%s)\n",
