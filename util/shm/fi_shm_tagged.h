	...
	TAILQ_FOREACH(request_iterator, &(ep->unexpected_recv_request_queue), requests)
	{
		...
		if (!((request_iterator->tag ^ tag) & (~ignore)))
		{
			...
			else
			{
				...
				if (request_iterator->connection != (fi_shm_connection_t *)FI_ADDR_UNSPEC)
				{
					if (!request_iterator->connection->activity_counter)
					{
						LIST_INSERT_HEAD(&(ep->active_connection_list),
								 request_iterator->connection,
								 active_connections);
					}

					request_iterator->connection->activity_counter++;
				}
			}
